...
def S():
    ...
    s['rubric']        = ParagraphStyle('rb',  fontName='RL-SB', fontSize=8.3,  textColor=PRUNE_LIGHT,      leading=12,  letterSpacing=2,   spaceAfter=2)
    s['body']          = ParagraphStyle('bd',  fontName='RL',    fontSize=9.5,  textColor=PRUNE_LIGHT,      leading=16)
    ...
    s['vigil_body']    = ParagraphStyle('vb',  fontName='RL',    fontSize=9,    textColor=PRUNE_LIGHT,      leading=14)
...
def bg_cover(canvas, doc):
    ...
    canvas.setFont('CG-I', 12); canvas.setFillColor(PRUNE_LIGHT)
...
def bg_interior(canvas, doc):
    ...
    canvas.setFont('RL', 7.5); canvas.setFillColor(PRUNE_LIGHT)
    canvas.drawRightString(PAGE_W - MR, 12 * mm, str(doc.page))
    canvas.setFont('RL', 7.5); canvas.setFillColor(colors.HexColor('#D9C9C3'))
